...
fn main() {
    ...
    let mut prio1 = Priority::RealTime;
    let mut prio2 = Priority::Background;
    if let Some(p) = args.priority {
        prio1 = p.try_into().unwrap();
        prio2 = p.try_into().unwrap();
    }

    let payload_size = args.payload_size;

    let data1: ZBytes = (0..payload_size)
        .map(|i| (i % 10) as u8)
        .collect::<Vec<u8>>()
        .into();

    let data2: ZBytes = (0..payload_size)
        .map(|i| ((i + 1) % 10) as u8)
        .collect::<Vec<u8>>()
        .into();
    ...
    loop {
        publisher1.put(data1.clone()).wait().unwrap();

        publisher2.put(data2.clone()).wait().unwrap();
